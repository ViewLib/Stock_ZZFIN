...
- (NSArray *)convertToPositionModelsWithXPosition:(CGFloat)startX drawLineModels:(NSArray *)drawLineModels  maxValue:(CGFloat)maxValue minValue:(CGFloat)minValue {
    if (!drawLineModels) return nil;

    _drawLineModels = drawLineModels;
    [self.Positions removeAllObjects];
    [self.NewPoint removeAllObjects];
    ...
    [drawLineModels enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (selfWeak.getNewsData) {
            if (selfWeak.getNewsData(obj[@"day"])) {
